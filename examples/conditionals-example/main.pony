...
actor Main
  new create(env: Env) =>
    // Simple if/else: show different text based on whether a value
    // exists
    let greeting =
      try
        Template.parse(
          "{{ if name }}Hello {{ name }}!" +
          "{{ else }}Hello stranger!{{ end }}")?
      else
    ...
    // "admin panel"
    try env.out.print(role_msg.render(admin_values)?) end

    let member_values = TemplateValues
    member_values("member") = "yes"
    // "member area"
    try env.out.print(role_msg.render(member_values)?) end
    ...
    // Sequence truthiness: `if` checks both existence and
    // non-emptiness. Use `if` to guard a loop so it only renders when
    // items are present.
    let items_list =
      try
        Template.parse(
