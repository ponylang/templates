...
class PrintingSink is TemplateSink
  """
  A sink that prints each segment with a label, demonstrating the
  alternating literal/dynamic_value call pattern.
  """
  let _env: Env
...
actor Main
  new create(env: Env) =>
    ...
    try
      ...
      (let statics, let dynamics) =
        template.render_split(values)?
      env.out.print(
        "Statics (" + statics.size().string() + "):")
      for (i, s) in statics.pairs() do
        env.out.print(
          "  [" + i.string() + "] \"" + s + "\"")
      end
      env.out.print(
        "Dynamics (" + dynamics.size().string() + "):")
    ...
    try
      let html_template =
        HTMLTemplate.parse("<p>{{ message }}</p>")?
      ...
      env.out.print("")
      env.out.print(
        "--- HTMLTemplate render_split " +
        "(escaped dynamics) ---")
      (let statics, let dynamics) =
        html_template.render_split(html_values)?
      for (i, s) in statics.pairs() do
        env.out.print(
          "  static[" + i.string() + "]: \"" + s + "\"")
      end
