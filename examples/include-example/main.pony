// This example demonstrates how to use partials (includes) to reuse
// template fragments across templates
...
actor Main
  new create(env: Env) =>
    // Partials are raw template strings registered in a
    // TemplateContext. They can contain any template syntax —
    ...
    // Use {{ include "name" }} to inline a partial. The partial
    // shares the same variable scope as the surrounding template.
    let template =
      try
        Template.parse(
          "{{ include \"header\" }}\n" +
